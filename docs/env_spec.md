...
① Softmax → w_new(∑=1)          ② r_log = returns_df.iloc[t]
    ...
    └──────────┬────────────────────────┘
               ▼
④ src.reward.calculate_reward_verbose(w_prev, w_new, r_arith, c)
               │
               ▼
...
세 배열의 shape이 서로 다르면 `ValueError`. 세 함수 사용처가 정확히 같은 A를 쓴다는 계약(env·백테스트 엔진 모두 `config.assets` 길이).
